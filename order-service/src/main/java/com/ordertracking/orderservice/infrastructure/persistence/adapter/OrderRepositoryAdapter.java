...
public class OrderRepositoryAdapter implements OrderRepository {
    ...
    public Order save(Order order) {
        return jpaRepository.findById(order.getId().value())
            .map(existing -> {
                // Update mutable fields on the managed entity to avoid deleting
                // and recreating order items on every status transition.
                mapper.updateEntity(order, existing);
                return mapper.toDomain(jpaRepository.save(existing));
            })
            .orElseGet(() -> mapper.toDomain(jpaRepository.save(mapper.toEntity(order))));
    }
